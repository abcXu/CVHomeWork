...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=2, shift_size=0,
        ...
        # print("shift",shift_size)
        # shift_size 取0或window_size//2
        if self.shift_size > 0:
            # calculate attention mask for SW-MSA
            H, W = self.input_resolution  # 32，32 img_size//patch_size
...
class SwinLSTM(nn.Module):
    def __init__(self, img_size, patch_size, in_chans, embed_dim, depths_downsample, depths_upsample, num_heads,
        ...
        in_chans:      输入的维度
        embed_dim:      隐藏层的维度 128
        depth_downsample: 上采样的层数
        depth_upsample:  下采样的层数
        num_heads:    多头注意力分头的个数  
        window_size:  窗口的大小
        """
...
if __name__=="__main__":
    ...
    states_down = [None] * len(args.depths_down)
    states_up = [None] * len(args.depths_up)
    # print("states_down", states_down)  # ([8, 1, 64, 64])
    # states = [None] * len(args.depths)
    states_down, states_up,output = model(x1, states_down,states_up)
    # print("states_down",states_down.shape)  # ([8, 1, 64, 64])
    # print("states_up", states_up.shape)  # ([8, 1, 64, 64])

...
class Moving_MNIST(Dataset):
    ...
    def __init__(self, args, split):
        ...
        with gzip.open(args.train_data_dir, 'rb') as f:
            ...
            self.datas = self.datas.reshape(-1, *args.image_size)   # (60000, 28, 28)
        ...
        self.step_length = args.step_length   # default 0.1
        self.num_objects = args.num_objects   # default=[2]
    ...
    def _get_random_trajectory(self, seq_length):  # 在二维空间中生成一个随机轨迹
    ...
    def _generate_moving_mnist(self, num_digits=2):  # 生成动态的mnist
...
class Moving_MNIST_Test(Dataset):
    ...
    def __getitem__(self, index):
        # datasets [20,10000,64,64,1]
        images = self.dataset[:, index, ...]   # [20,64,64,1]
...
if __name__=="__main__":
    ...
    args = get_args()
    dataset = Moving_MNIST(args, 'train')
    print(dataset[0][0].shape)  # input (10,1,64,64)
    print(dataset[0][1].shape)   # target (10,1,64,64)

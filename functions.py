...
def model_forward_multi_layer(model, inputs, targets_len, num_layers):
    ...
    :param model:训练的模型
    :param inputs: [B,T,C,H,W]
    :param targets_len: 10,输入序列的长度
    :param num_layers: [2,6],长度代表下采样或者上采样的层数，里面的值代表每一层的stb数量
    :return: list 长度19,每个元素[B,C,H,W]的tensor  ->  [19，8，1，64，64]
    """
...
def train(args, logger, epoch, model, train_loader, criterion, optimizer):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        with autocast():
            ...
            # outputs: [19,8,1,64,64] -> [8,19,1,64,64]
            outputs = torch.stack(outputs).permute(1, 0, 2, 3, 4).contiguous()
            # outputs: [8, 19, 1, 64, 64]
            # targets_ [8, 19, 1, 64, 64],去掉第一个输入,因为没有与他相对的预测值
